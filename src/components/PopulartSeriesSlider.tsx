...
function PopularSeriesSlider() {
  const history = useHistory();
  const bigBoxMatch = useRouteMatch<{ seriesId: string }>(
    "/series/popular/:seriesId"
  );
  ...
  const onBoxClicked = (seriesId: number) => {
    history.push(`/series/popular/${seriesId}`);
  };
    ...
    <>
      <Slider>
        <AnimatePresence
          ...
          custom={{ isNext }}
        >
          <SliderTitle>인기 컨텐즈</SliderTitle>
          <SliderBox
            variants={sliderVar}
            initial="hidden"
            animate="visible"
        ...
        {bigBoxMatch ? (
          <>
            ...
            <BigBox
              ...
              layoutId={"popular" + bigBoxMatch.params.seriesId}
            >
              {clickedSeries && (
                <>
                  <BigImg
