...
function TopMovieSlider() {
  ...
  const topMovieMatch = useRouteMatch<{ movieId: string }>(
    "/topmovie/:movieId"
  );
  ...
  const { data: movieData, isLoading: movieIsLoading } = useQuery({
    queryKey: ["movie", topMovieMatch?.params.movieId],
    queryFn: () => {
      const result = getMovie(Number(topMovieMatch?.params.movieId));
      return result;
    },
    enabled: typeof topMovieMatch?.params.movieId === "string",
  });
  ...
  const clickedTopMovie =
    ...
    data?.results.find((movie) => movie.id === +topMovieMatch.params.movieId);
        ...
        {topMovieMatch ? (
          <>
            ...
            <BigBox
              ...
              layoutId={"Top" + topMovieMatch.params.movieId}
            >
              {clickedTopMovie && (
                <>
                  <BigImg
                    style={{
                      ...
                      url(${makeImagePath(clickedTopMovie.backdrop_path)})`,
                    }}
                  ></BigImg>

                  <BigTitle>
                    <span>{clickedTopMovie.title}</span>
