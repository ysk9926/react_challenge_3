...
function UpComingMovieSlider() {
  ...
  const { data: movieData, isLoading: movieIsLoading } = useQuery({
    queryKey: ["movie", upComingMovieMatch?.params.movieId],
    queryFn: () => {
      const result = getMovie(Number(upComingMovieMatch?.params.movieId));
      return result;
    },
    enabled: typeof upComingMovieMatch?.params.movieId === "string",
  });
  ...
  const onBoxClicked = (movieId: number) => {
    history.push(`/upcomingmovie/${movieId}`);
  };
    ...
    <>
      <Slider>
        <AnimatePresence
          ...
          custom={{ isNext }}
        >
          <SliderTitle>개봉 콘텐츠</SliderTitle>
          <SliderBox
            variants={sliderVar}
            initial="hidden"
            animate="visible"
        ...
        {upComingMovieMatch ? (
          <>
            ...
            <BigBox
              ...
              layoutId={"upcoming" + upComingMovieMatch.params.movieId}
            >
              {clickedTopMovie && (
                <>
                  <BigImg
